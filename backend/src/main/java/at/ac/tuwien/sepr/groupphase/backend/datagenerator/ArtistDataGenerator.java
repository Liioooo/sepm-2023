...
public class ArtistDataGenerator extends DataGenerator<Artist> {
    ...
    public ArtistDataGenerator(ArtistRepository artistRepository, Environment environment) {
    ...
    protected List<Artist> generate() {
        ...
        List<Artist> autoGeneratedArtists = new ArrayList<>();
        for (int i = 0; i < 10; i++) {
            autoGeneratedArtists.add(Artist.builder()
                .fictionalName(faker.artist().name())
                .build());
        }
        artists.addAll(autoGeneratedArtists);
        return artistRepository.saveAll(artists);
    }
}
